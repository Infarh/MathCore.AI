...
namespace MathCore.AI.Tests.NeuralNetworks
{
    ...
    public class MultilayerPerceptron_Tests
    {
        ...
        [TestMethod]
        public void SingleNeuronNetwork0()
        {
            ...
            var error = data
               .Mix()
               .Select(d => teacher.Teach(new[] { d.x }, output, new[] { d.f }, 0.03))
               .TakeWhile(e => e > 0)
               .ToArray();


            Assert.That.Value(k).IsEqual(2, 1e-10);
            Assert.That.Value(b).IsEqual(5, 1e-10);
        }
