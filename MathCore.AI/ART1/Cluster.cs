...
namespace MathCore.AI.ART1
{
    ...
    public sealed class Cluster<T> : IEnumerable<T>
    {
        ...
        internal Cluster([NotNull] double[] FeatureVector, T Item, [NotNull] Func<T, double[]> Classificator, [NotNull] string[] FeatureNames)
        {
        ...
        [NotNull] private string ToDebuggerString() => $"{this}:{string.Join(", ", _Items)}";
