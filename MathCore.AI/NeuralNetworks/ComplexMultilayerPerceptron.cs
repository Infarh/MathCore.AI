...
public class ComplexMultilayerPerceptron : IComplexNeuralNetwork
{
    ...
    public sealed class LayerManager
    {
        ...
        private readonly Complex[][,] _Layers;
        private readonly Complex[][] _Outputs;
        ...
        public ComplexActivationFunction? Activation { get => _Activations[LayerIndex]; set => _Activations[LayerIndex] = value; }

        /// <summary>Смещения слоя</summary>
        public Complex[] Offsets => _Offsets[LayerIndex];

        /// <summary>Коэффициенты смещений слоя</summary>
        public Complex[] OffsetWeights => _OffsetWeights[LayerIndex];

        public Complex[] Outputs => _Outputs[LayerIndex];

        /// <summary>Матрица коэффициентов передачи входов слоя</summary>
        public Complex[,] Weights => _Layers[LayerIndex];
        ...
        internal LayerManager(int LayerIndex,
            Complex[][,] Layers,
            Complex[][] Offsets,
            Complex[][] OffsetWeights,
            Complex[][] Outputs,
            ComplexActivationFunction[] Activations)
        {
        ...
        public void SetOffsets(ICollection<Complex> OffsetValues) => (OffsetValues.NotNull()).CopyTo(Offsets, 0);
        ...
        public void SetOffsetWeights(ICollection<Complex> WeightValues) => (WeightValues.NotNull()).CopyTo(OffsetWeights, 0);
        ...
        public void SetOutputs(ICollection<Complex> OutputValues) => (OutputValues.NotNull()).CopyTo(Outputs, 0);
    ...
    protected readonly Complex[][] _OffsetsWeights;

    /// <summary>Массив выходов скрытых слоёв</summary>
    protected readonly Complex[][] _Outputs;

    /// <summary>Функции активации слоёв</summary>
    protected readonly ComplexActivationFunction[] _Activations;
    ...
    public Complex[,] this[int layer] => _Layers[layer];

    /// <summary>Смещения слоёв</summary>
    public IReadOnlyList<Complex[]> Offests => _Offsets;

    /// <summary>Весовые коэффициенты смещений слоёв</summary>
    public IReadOnlyList<Complex[]> OffsetWeights => _OffsetsWeights;
    ...
    public ComplexMultilayerPerceptron(params Complex[][,] Layers)
    {
        _Layers = Layers.NotNull();
    ...
    public ComplexMultilayerPerceptron(
        ...
        IEnumerable<int> NeuronsCount,
        Random rnd)
        : this(CreateLayersMatrix(InputsCount, NeuronsCount, (_, _, _) => rnd.NextDouble())) { }
    ...
    {
        if (Input.NotNull().Length != InputsCount) throw new ArgumentException($"Размер входного вектора ({Input.Length}) не равен количество входов сети ({InputsCount})", nameof(Input));
        if (Output.NotNull().Length != OutputsCount) throw new ArgumentException($"Размер выходного вектора ({Output.Length}) не соответствует количеству выходов сети ({OutputsCount})", nameof(Output));
